...
| `self-guarded` | Inject a checklist before model requests so the model self-evaluates tool calls. Runtime still blocks deny rules and asks the user on uncertain calls. |
...
`self-guarded` is tuned for local models. By default it refreshes a checklist before each model request, so the model sees the guard right before deciding whether to call tools.

Checklist timing is configured in this extension's settings file:
...
```text
always -> refresh before each model request, default
turn   -> attach once at the start of each user turn, lower token usage
```

The checklist tells the model to evaluate task relevance, cwd scope, shell risk, custom tool effects, configured deny/ask/allow rules, and uncertainty before calling a tool.
